...
async def get_github_overview(
    ...
    # 2. Open PRs this sprint (created within sprint window, not yet merged/closed)
    open_prs = (
        await db.execute(
            select(func.count())
    ...
    # 3. Merged PRs this sprint (merged within sprint window)
    merged_prs = (
        await db.execute(
            select(func.count())
            .select_from(PullRequest)
...
async def get_github_activity(
    ...
    # Team members for the developer dropdown
